...
import { UpdateSectionSchema } from './UpdateSectionSchema';

const UpdateSection = ({ data, getData}) => {
  // Global State
  ...
  const onSubmit = async (values, actions) => {
    try {
      const res = await axios.put(`${BASE_URL}section/updateOne/${data.id}`, values, {
        headers: {
    ...
    <form
      ...
      <div className='flex'>
        <input
          ...
          type="text"
          name='slug'
          placeholder='الرمز الخاص بالسنة'
          value={values.slug}
          onChange={handleChange}
          onBlur={handleBlur}
        />
      ...
      </div>
      {
        ...
        <p className='error text-[#dc3545]'>{errors.slug}</p>
      }
      <button
